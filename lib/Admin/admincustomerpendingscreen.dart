...
class AdminCustomerPendingOrdersScreen extends StatelessWidget {
  ...
  const AdminCustomerPendingOrdersScreen({Key? key, required this.customerName}) : super(key: key);
    ...
    return Scaffold(
      ...
      body: FutureBuilder<QuerySnapshot>(
        future: FirebaseFirestore.instance
            ...
            .limit(1)  // To make sure we only get one customer document
            .get()
            .then((customerQuerySnapshot) {
          if (customerQuerySnapshot.docs.isNotEmpty) {
            return FirebaseFirestore.instance
                .collection('Customers')
                .doc(customerQuerySnapshot.docs.first.id)
                .collection('myorders')  // Use 'myorders' instead of 'orders'
                .get();
          } else {
            return Future.error('No customer found with the given name');
          }
        }),
            ...
            return SingleChildScrollView(
              child: Column(
                children: List.generate(orders.length, (index) {
                  ...
                  final phone = orderData['phoneNumber'] as String?;  // Corrected field name
  ...
  void moveDataToTargetCollectionRejected(DocumentSnapshot orderDoc, String rejectionReason) async {
    final orderData = orderDoc.data() as Map<String, dynamic>;
